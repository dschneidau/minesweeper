...
numOfBombs =2100 #Number of bombs
numOfRows = 38 #Number of rows
numOfCols = 200 #Number of columns
...
def FlagClick(x,y,index):
	...
	if isClickedList[index]==0:
		if isFlaggedList[index]==0:
			...
			btnBox.config(bg="#dcca98")
			isFlaggedList[index] = 1
		else:
			if isFlaggedList[index]==1:
				btnBox.config(text="")
				btnBox.config(bg="38afcd")
				btnBox.config(fg="38afcd")
...
for i in range(numOfRows):
	for j in range(numOfCols):
		btn.append(Button(root, width=1, height=1, font='Terminal', bg="#000000", relief="flat"))
